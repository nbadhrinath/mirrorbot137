...
class QbitTorrent:
    ...
    def add_torrent(self, link, dire, listener, qbitsel):
        ...
        try:
            ...
            if len(tor_info) > 0:
                sendMessage("👁️‍🗨️This Torrent is already in list👁️‍🗨️", listener.bot, listener.update)
                ...
                if len(tor_info) == 0:
                    while True:
                        if time.time() - self.meta_time >= 20:
                            sendMessage("⚠️The Torrent was not added. Report when you see this error⚠️", listener.bot, listener.update)
            ...
            tor_info = tor_info[0]
            LOGGER.info(f"☑️QbitDownload started: {tor_info.name}")
        ...
        try:
            ...
            if tor_info.state == "metaDL":
                ...
                if time.time() - self.stalled_time >= 999999999: # timeout after downloading metadata
                    self.client.torrents_pause(torrent_hashes=self.ext_hash)
                    time.sleep(0.3)
                    self.listener.onDownloadError("☠️Dead Torrent!")
                    self.client.torrents_delete(torrent_hashes=self.ext_hash)
                    self.client.auth_log_out()
                    self.updater.cancel()
