...
class GoogleDriveHelper:
    ...
    def __upload_empty_file(self, path, file_name, mime_type, parent_id=None):
        ...
        file_metadata = {
            'name': file_name,
            'description': 'Uploaded using Mirrorbot137',
            'mimeType': mime_type,
        }
    ...
    def drive_list(self, fileName, stopDup=False, noMulti=False):
        ...
        for index, parent_id in enumerate(DRIVES_IDS):
            ...
            if not response["files"] and noMulti:
                ...
                continue
            if not Title:
                msg += f'<h4>⚜️Search Results For: {fileName}</h4>👇<br><br>'
                Title = True
            ...
            for file in response.get('files', []):
                if file.get('mimeType') == "application/vnd.google-apps.folder":
                    ...
                    if SHORTENER is not None and SHORTENER_API is not None:
                        sfurl = short_url(furl)
                        msg += f"<b><a href={sfurl}>☁️ Drive Link ☁️</a></b>"
                    else:
                        msg += f"<b><a href={furl}>☁️ Drive Link ☁️</a></b>"
                    if INDEX_URLS[index] is not None:
                        if RECURSIVE_SEARCH:
        ...
        msg = f"<b>⚜️Found <code>{all_contents_count}</code> results for <code>{fileName}👇</code></b>"
        buttons = button_build.ButtonMaker()
